...
class ResNet(nn.Module):
    ...
    def forward(self, x, return_features=False):
        ...
        x1 = self.layer3(x)
        x1 = self.aspp(x1)
        x2 = torch.cat((x1,self.layer4(x1)),dim=1)
        x2 = self.aspp(x2)
        if return_features:
            x1, features1 = self.decoder(x1, low_level_feat, True)
            x2, features2 = self.decoder(x2, low_level_feat, True)
            return x1,x2, features1,features2
        else:
