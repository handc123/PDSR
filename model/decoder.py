...
class dec_deeplabv3_plus(nn.Module):
    def __init__(
        ...
        if self.rep_head:
            ...
            self.representation2 = nn.Sequential(
                nn.Conv2d(512, 256, kernel_size=3, stride=1, padding=1, bias=True),
                norm_layer(256),
                nn.ReLU(inplace=True),
                nn.Dropout2d(0.1),
    ...
    def forward(self, x):
        ...
        res1 = {"pred": self.classifier1(low_feat)}
        ...
        res2 = {"pred": self.classifier2(aspp_out)}

        if self.rep_head:
            res1["rep"] = self.representation1(low_feat)
            res2["rep"] = self.representation2(aspp_out)
